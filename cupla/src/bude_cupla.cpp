...
//Define the new cupla functions before usage
struct fasten_main;
//struct compute_transformation_matrix;
...
void runCUPLA(float* results)
{
  ...
  {
     CUPLA_KERNEL_OPTI(fasten_main)(global, local, shared)(
        params.natlig, 
        params.natpro,
        _cuda.d_protein,
        _cuda.d_ligand,
    ...
    const int numTransforms) const
{
...
#ifdef USE_SHARED
  //Replace the shared memory with a plain pointer of type FFParams
  sharedMem(forcfield, FFParams);
  ...
  //Inlining this function to reduce calls between device functions
  sx = sin(transforms_0[index]);
  cx = cos(transforms_1[index]);
  sy = sin(transforms_2[index]);
  cy = cos(transforms_3[index]);
  sz = sin(transforms_4[index]);
  cz = cos(transforms_5[index]);

  transform[i]->x = cy*cz;
  transform[i]->y = sx*sy*cz - cx*sz;
  transform[i]->z = cx*sy*cz + sx*sz;
  transform[i]->w = transforms_3[index];
