...
int main(int argc, char *argv[])
{
  ...
  printf("\n Reference        CUPLA   (diff)\n");
  for (int i = 0; i < n_ref_poses; i++)
  {
    if (fabs(resultsRef[i]) < 1.f && fabs(resultsCUPLA[i]) < 1.f) continue;

    float diff = fabs(resultsRef[i] - resultsCUPLA[i]) / resultsCUPLA[i];
    if (diff > maxdiff) {
      maxdiff = diff;
      // printf ("Maxdiff: %.2f (%.3f vs %.3f)\n", maxdiff, resultsRef[i], resultsCUPLA[i]);
    }

    if (i < 8)
      printf("%7.2f    vs   %7.2f  (%5.2f%%)\n", resultsRef[i], resultsCUPLA[i], 100*diff);
  }
